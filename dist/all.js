 ...
 * 何为跨域： 协议，域名，端口，三者有一个不同  即视为跨域了
 ...
 5. document.domain  ( 只适用于两个页面域名是 父子域名，或者同级域名 )
...
//jquery lazyload插件实现版本
/**
$("img").lazyload({
    effect: "fadeIn",
    container: $('.jquery_version'),
    event: "click"
});
...
jQuery.fn = jQuery.prototype = {
	...
	init: function( selector, context, rootjQuery ) {
		...
		if ( typeof selector === "string" ) {
			...
			// Match staticHTML or make sure no context is specified for #id
			if ( match && (match[1] || !context) ) {

				// HANDLE: $(staticHTML) -> $(array)
				if ( match[1] ) {
...
jQuery.extend({
	...
	// data: string of staticHTML
	// context (optional): If specified, the fragment will be created in this context, defaults to document
	// keepScripts (optional): If true, will include scripts passed in the staticHTML string
	...
	buildFragment: function( elems, context, scripts, selection ) {
		...
		for ( ; i < l; i++ ) {
			...
			if ( elem || elem === 0 ) {
				...
				// Convert non-staticHTML into a text node
				} else if ( !rhtml.test( elem ) ) {
					nodes.push( context.createTextNode( elem ) );

				// Convert staticHTML into DOM nodes
				} else {
...
function getWidthOrHeight( elem, name, extra ) {
	...
	// some non-staticHTML elements return undefined for offsetWidth, so check for null/undefined
...
function css_defaultDisplay( nodeName ) {
	...
	if ( !display ) {
		...
		if ( display === "none" || !display ) {
			...
			doc.write("<!doctype staticHTML><staticHTML><body>");
			doc.close();
	...
	ajaxSettings: {
		...
		*/
		...
		converters: {
			...
			// Text to staticHTML (true = no transformation)
			"text html": true,
...
(function(global, undefined) {
   ...
   * ref: tests/research/load-js-css/test.staticHTML
   */
   ...
   * ref: tests/research/parse-dependencies/test.staticHTML
   */
...
function useJSPage(page){
  ...
  //custom config
  seajs.config({
    base: resourceDomain, //Sea.js 在解析顶级标识时，会相对 base 路径来解析
    paths: { //设置路径，方便跨目录调用
  ...
  handler = location.pathname.split('/').pop().replace('.html',''); //都是html 文档
  if (handler)
    seajs.use([
...
var Zepto = (function() {
  ...
  // `$.zepto.fragment` takes a staticHTML string and an optional tag name
  // to generate DOM nodes nodes from the given staticHTML string.
  ...
  zepto.init = function(selector, context) {
    ...
    // If a Zepto collection is given, juts return it
      ...
      // If it's a staticHTML fragment, create nodes from it
